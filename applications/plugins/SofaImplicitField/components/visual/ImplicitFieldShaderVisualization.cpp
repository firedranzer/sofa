...
void ImplicitFieldShaderVisualization::drawVisual(const VisualParams* vp)
{
    ...
    /// Create an orthomatrix to have the triangle filling the whole space.
    //glMatrixMode(GL_PROJECTION);
    //glPushMatrix();
    //glLoadIdentity();

    //auto& viewport = vp->viewport() ;
    //gluOrtho2D(-1, 1, -1, 1 );
    ...
    //double aspectRatio = vp->viewport()[2]/vp->viewport()[3] ;
    //std::cout << "Z" << vp->zNear() << " far " << vp->zFar() << std::endl ;
