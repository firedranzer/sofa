...
cpdef Shape2D closedPolygonalChain(list listOfCouplesxy):
    ...
    newShape=HalfPlaneGivenByAVector2D(u)
    ...
    for i in range(1,length):
        ...
        if Det(u,v)<=0.0:
            ...
            newShape=Intersection(newShape, HalfPlaneGivenByAVector2D(v))
        else:

            newShape=Union(newShape, HalfPlaneGivenByAVector2D(v))

        u=v

    newShape=Intersection(newShape, HalfPlaneGivenByAVector2D(u))
